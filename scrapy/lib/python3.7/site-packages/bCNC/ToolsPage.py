...
class _Base:
	...
	def event_generate(self, msg, **kwargs):
		self.main.listbox.event_generate(msg, **kwargs)
	...
	# ----------------------------------------------------------------------
	def populate(self):
		self.main.listbox.delete(0,END)
		for var in self.variables:
			...
			elif t == "mm" and self.main.inches:
				try:
					value /= 25.4
					value = round(value, self.main.digits)
				except:
					value = ""
			elif t == "float":
				try:
			...
			self.main.listbox.insert(END, (l, value))

			if t=="color":
				try:
					self.main.listbox.listbox(1).itemconfig(END, background=value)
				except TclError:
					pass
		...
		self.main.widget['paned'].remove(self.main.widget['toolHelpFrame'])
		self.main.widget['toolHelp'].config(state=NORMAL)
		self.main.widget['toolHelp'].delete(1.0, END)
		if len(varhelp) > 0:
			for line in varhelp.splitlines():
				if len(line) > 0 and line[0] == '#' and line[1:] in Utils.images.keys():
					self.main.widget['toolHelp'].image_create(END,image=Utils.images[line[1:]])
					self.main.widget['toolHelp'].insert(END, '\n')
				else:
					self.main.widget['toolHelp'].insert(END, line+'\n')
			self.main.widget['paned'].add(self.main.widget['toolHelpFrame'])
		self.main.widget['toolHelp'].config(state=DISABLED)

	#----------------------------------------------------------------------
	def _sendReturn(self, active):
		self.main.listbox.selection_clear(0,END)
		self.main.listbox.selection_set(active)
		self.main.listbox.activate(active)
		self.main.listbox.see(active)
		n, t, d, l = self.variables[active][:4]
		if t=="bool": return	# Forbid changing value of bool
		self.main.listbox.event_generate("<Return>")

	#----------------------------------------------------------------------
	def _editPrev(self):
		active = self.main.listbox.index(ACTIVE)-1
		if active<0: return
	...
	def _editNext(self):
		active = self.main.listbox.index(ACTIVE)+1
		if active>=self.main.listbox.size(): return
		self._sendReturn(active)
	...
	def edit(self, event=None, rename=False):
		lb = self.main.listbox.listbox(1)
		...
		self.main.listbox.selection_set(active)
		self.main.listbox.activate(active)
		self.main.listbox.yview_moveto(ypos)
	...
	def fromMm(self, name, default=0.0):
		try:
			return self.main.fromMm(float(self[name]))
		except ValueError:
			return default
...
class DataBase(_Base):
	...
	def rename(self):
		...
		self.main.listbox.activate(0)
		self.main.listbox.see(0)
		self.edit(None,True)
...
class Plugin(DataBase):
	def __init__(self, main, name):
		DataBase.__init__(self, main, name)
		self.plugin = True
		self.group  = "Macros"
		self.oneshot = False
		self.help = None
...
#==============================================================================
class Ini(_Base):
	def __init__(self, main, name, vartype, include=(), ignore=()):
		_Base.__init__(self, main)
		self.name = name
...
class Font(Ini):
	def __init__(self, main):
		Ini.__init__(self, main, "Font", "str")


#------------------------------------------------------------------------------
class Color(Ini):
	def __init__(self, main):
		Ini.__init__(self, main, "Color", "color")


#------------------------------------------------------------------------------
class Events(Ini):
	def __init__(self, main):
		Ini.__init__(self, main, "Events", "str")


#------------------------------------------------------------------------------
class Shortcut(_Base):
	def __init__(self, main):
		_Base.__init__(self, main, "Shortcut")
		self.variables = [
			("F1",		"str",	"help"	, _("F1")),
			("F2",		"str",	"edit"	, _("F2")),
			("F3",		"str",	"XY"	, _("F3")),
...
class Camera(_Base):
	def __init__(self, main):
		_Base.__init__(self, main, "Camera")
		self.variables = [
...
class Config(_Base):
	...
	# ----------------------------------------------------------------------
	def update(self):
		self.main.inches        = self["units"]
		self.main.digits        = int(self["round"])
		self.main.cnc().decimal = self.main.digits
		self.main.cnc().startup = self["startup"]
		self.main.gcode.header  = self["header"]
		self.main.gcode.footer  = self["footer"]
		return False
...
class Material(DataBase):
	def __init__(self, main):
		DataBase.__init__(self, main, "Material")
		self.variables = [
			("name",    "db",    "", _("Name")),
	...
	def update(self):
		...
		stockmat = self.main["stock"]["material"]
		if stockmat=="" or stockmat==self["name"]:
			self.main.cnc()["cutfeed"]  = self.fromMm("feed")
			self.main.cnc()["cutfeedz"] = self.fromMm("feedz")
			self.main.cnc()["stepz"]    = self.fromMm("stepz")
		return False
...
class EndMill(DataBase):
	def __init__(self, main):
		DataBase.__init__(self, main, "EndMill")
		self.variables = [
			("name",       "db",     "", _("Name")),
			("comment",   "str",     "", _("Comment")),
			("type",     "list",     "", _("Type")),
	...
	# ----------------------------------------------------------------------
	def update(self):
		self.main.cnc()["diameter"] = self.fromMm("diameter")
		self.main.cnc()["stepover"] = self["stepover"]
		return False
...
class Stock(DataBase):
	def __init__(self, main):
		DataBase.__init__(self, main, "Stock")
		self.variables = [
			("name",      "db" ,    "", _("Name")),
			("comment",  "str",     "", _("Comment")),
	...
	# ----------------------------------------------------------------------
	def update(self):
		self.main.cnc()["safe"]      = self.fromMm("safe")
		self.main.cnc()["surface"]   = self.fromMm("surface")
		self.main.cnc()["thickness"] = self.fromMm("thickness")
		if self["material"]:
			self.main["material"].makeCurrent(self["material"])
		return False
...
class Cut(DataBase):
	def __init__(self, main):
		DataBase.__init__(self, main, "Cut")
		self.icon = "cut"
		self.variables = [
			("name",         "db" ,    "", _("Name")),
...
'''
	...
	def execute(self, app):
		...
		if strategy in ['ramp+bottom','ramp']:
			helixBottom = True
			ramp = self["ramp"]
			if ramp < 0: ramp = self.main.fromMm(float(ramp))
...
class Drill(DataBase):
	def __init__(self, main):
		DataBase.__init__(self, main, "Drill")
		self.icon = "drill"
		self.variables = [
			("name",      "db" ,    "", _("Name")),
...
class Profile(DataBase):
	def __init__(self, main):
		DataBase.__init__(self, main, "Profile")
		self.icon = "profile"
		self.variables = [
			("name",      "db" ,    "", _("Name")),
...
'''
	...
	def execute(self, app):
		if self["endmill"]:
			self.main["endmill"].makeCurrent(self["endmill"])
...
class Pocket(DataBase):
	def __init__(self, main):
		DataBase.__init__(self, main, "Pocket")
		self.icon = "pocket"
		self.variables = [
			("name",      "db" ,    "", _("Name")),
...
class Tabs(DataBase):
	def __init__(self, main):
		DataBase.__init__(self, main, "Tabs")
		self.icon = "tab"
		self.variables = [
			("name",      "db" ,    "", _("Name")),
...
'''
	...
	def execute(self,app):
		...
		dx    = self.fromMm("dx", self.main.fromMm(5.))
		#dy    = self.fromMm("dy", self.main.fromMm(5.))
		dy = dx
		z     = self.fromMm("z", -self.main.fromMm(3.))
...
class DataBaseGroup(CNCRibbon.ButtonGroup):
	def __init__(self, main, app):
		CNCRibbon.ButtonGroup.__init__(self, main, N_("Database"), app)
		self.grid3rows()
...
class CAMGroup(CNCRibbon.ButtonMenuGroup):
	def __init__(self, main, app):
		CNCRibbon.ButtonMenuGroup.__init__(self, main, N_("CAM"), app)
		self.grid3rows()
...
#	def __init__(self, main, group, app):
#		CNCRibbon.ButtonGroup.__init__(self, main, group, app)
#		self.grid3rows()
#
#		col,row=0,0
#		# Find plugins in the plugins directory and load them
...
#	def __init__(self, main, app):
#		PluginsGroup.__init__(self, main, N_("Macros"), app)
#
#class GeneratorGroup(PluginsGroup):
#	def __init__(self, main, app):
#		PluginsGroup.__init__(self, main, N_("Generator"), app)
#
#class ArtisticGroup(PluginsGroup):
#	def __init__(self, main, app):
#		PluginsGroup.__init__(self, main, N_("Artistic"), app)
...
class ConfigGroup(CNCRibbon.ButtonMenuGroup):
	def __init__(self, main, app):
		#CNCRibbon.ButtonGroup.__init__(self, main, N_("Config"), app)
		CNCRibbon.ButtonMenuGroup.__init__(self, main, N_("Config"), app)
		self.grid3rows()
...
class ToolsFrame(CNCRibbon.PageFrame):
	def __init__(self, main, app):
		CNCRibbon.PageFrame.__init__(self, main, "CAM", app)
