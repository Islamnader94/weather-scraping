...
class LabelGroup(Frame):
	def __init__(self, main, name, command=None, **kw):
...
class LabelButton(Button, _KeyboardFocus):
	def __init__(self, main, recipient=None, event=None, **kw):
		Button.__init__(self, main, **kw)
...
class LabelCheckbutton(Checkbutton, _KeyboardFocus):
	def __init__(self, main, **kw):
		Checkbutton.__init__(self, main, **kw)
...
class LabelRadiobutton(Radiobutton, _KeyboardFocus):
	def __init__(self, main, **kw):
		Radiobutton.__init__(self, main, **kw)
		self.config(
...
class LabelCombobox(tkExtra.Combobox, _KeyboardFocus):
	def __init__(self, main, **kw):
		tkExtra.Combobox.__init__(self, main, **kw)
...
class MenuButton(Button, _KeyboardFocus):
	def __init__(self, main, menulist, **kw):
		Button.__init__(self, main, **kw)
		self.config(	relief           = FLAT,
				activebackground = _ACTIVE_COLOR,
				font             = _FONT,
	...
	#-----------------------------------------------------------------------
	@staticmethod
	def createMenuFromList(main, menulist):
		mainmenu = menu = Menu(main, tearoff=0, activebackground=_ACTIVE_COLOR)
...
class MenuGroup(LabelGroup):
	def __init__(self, main, name, menulist=None, **kw):
		LabelGroup.__init__(self, main, name, command=self._showMenu, **kw)
...
#	def __init__(self, main, name, **kw):
#		LabelGroup.__init__(self, main, name, **kw)
...
class Page:		# <--- should be possible to be a toplevel as well
	...
	#-----------------------------------------------------------------------
	# The tab page can change main if undocked
	#-----------------------------------------------------------------------
...
#		self.refresh()
	...
	#-----------------------------------------------------------------------
	def createPage(self):
		self.page = Frame(self.main._pageFrame)
		return self.page
	...
	def refreshUndoButton(self):
		...
		if self.canUndo():
			...
			state = DISABLED
		self.main.tool["undo"].config(state=state)
		self.main.tool["undolist"].config(state=state)

		if self.canRedo():
			state = NORMAL
		else:
			state = DISABLED
		self.main.tool["redo"].config(state=state)
...
#		subordinates = self._tabFrame.pack_subordinates()
#		try:
#			pos = subordinates.index(event.widget)-1
#		except ValueError:
#			if event.widget is self.dynamic:
#				pos = len(subordinates)-1
#			else:
#				return
#		if pos < 0: return	# Do not replace First tab
#		subordinates[pos].select()
#		#self.changePage()
#		subordinates[pos].focus_set()
#
#	#-----------------------------------------------------------------------
