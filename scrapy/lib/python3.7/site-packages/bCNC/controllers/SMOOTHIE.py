...
class Controller(_GenericController):
	...
	def executeCommand(self, oline, line, cmd):
		if line[0] in ( "help", "version", "mem", "ls",
				...
				"fire", "switch"):
			if self.main.serial:
				self.main.serial_write(oline+"\n")
			return True
		return False

	def hardResetPre(self):
		self.main.serial_write(b"reset\n")

	def hardResetAfter(self):
		time.sleep(6)

	def viewBuild(self):
		self.main.serial_write(b"version\n")
		self.main.sendGCode("$I")

	def grblHelp(self):
		self.main.serial_write(b"help\n")
	...
	def parseBracketAngle(self, line, cline):
		...
		if self.main.sio_wait and not cline and l[0] not in ("Run","Jog", "Hold"):
