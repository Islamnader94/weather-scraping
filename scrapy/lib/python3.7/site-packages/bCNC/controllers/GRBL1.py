...
class Controller(_GenericGRBL):
	...
	def jog(self, dir):
		self.main.sendGCode("$J=G91 %s F100000"%(dir))
	...
	def overrideSet(self):
		...
		if diff==0:
			...
			self.main.serial_write(OV_FEED_100)
		elif diff >= 10:
			self.main.serial_write(OV_FEED_i10)
			CNC.vars["_OvChanged"] = diff>10
		elif diff <= -10:
			self.main.serial_write(OV_FEED_d10)
			CNC.vars["_OvChanged"] = diff<-10
		elif diff >= 1:
			self.main.serial_write(OV_FEED_i1)
			CNC.vars["_OvChanged"] = diff>1
		elif diff <= -1:
			self.main.serial_write(OV_FEED_d1)
			CNC.vars["_OvChanged"] = diff<-1
		# Check rapid
		...
		if target == current:
			...
			self.main.serial_write(OV_RAPID_100)
		elif target == 75:
			self.main.serial_write(OV_RAPID_50)	# FIXME: GRBL protocol does not specify 75% override command at all
		elif target == 50:
			self.main.serial_write(OV_RAPID_50)
		elif target == 25:
			self.main.serial_write(OV_RAPID_25)
		# Check Spindle
		diff = CNC.vars["_OvSpindle"] - CNC.vars["OvSpindle"]
		if diff==0:
			pass
		elif CNC.vars["_OvSpindle"] == 100:
			self.main.serial_write(OV_SPINDLE_100)
		elif diff >= 10:
			self.main.serial_write(OV_SPINDLE_i10)
			CNC.vars["_OvChanged"] = diff>10
		elif diff <= -10:
			self.main.serial_write(OV_SPINDLE_d10)
			CNC.vars["_OvChanged"] = diff<-10
		elif diff >= 1:
			self.main.serial_write(OV_SPINDLE_i1)
			CNC.vars["_OvChanged"] = diff>1
		elif diff <= -1:
			self.main.serial_write(OV_SPINDLE_d1)
			CNC.vars["_OvChanged"] = diff<-1
	...
	def parseBracketAngle(self, line, cline):
		...
		if CNC.vars["state"] != fields[0] or self.main.runningPrev != self.main.running:
			self.main.controllerStateChange(fields[0])
		self.main.runningPrev = self.main.running
		...
		for field in fields[1:]:
			...
			if word[0] == "MPos":
				try:
					...
					self.main.log.put((self.main.MSG_RECEIVE, CNC.vars["state"]))
					break
			elif word[0] == "F":
				try:
				...
				try:
					...
					if 'S' in word[1]:
						if CNC.vars["state"] == 'Idle' and not self.main.running:
							print("Stream requested by CYCLE START machine button")
							self.main.event_generate("<<Run>>", when = 'tail')
						else:
							print("Ignoring machine stream request, because of state: ", CNC.vars["state"], self.main.running)
				except (ValueError,IndexError):
					break


		# Machine is Idle buffer is empty stop waiting and go on
		if self.main.sio_wait and not cline and fields[0] not in ("Run", "Jog", "Hold"):
			#if not self.main.running: self.main.jobDone() #This is not a good idea, it purges the controller while waiting for toolchange. see #1061
